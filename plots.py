...
def draw_pitch(figure_width=700, figure_height=350,
    ...
    # middle of pitch
    p.circle(pitch_width / 2., y=pitch_height / 2., size=100, fill_color=pitch_color, line_width=2,
...
def plot_events(player_events, event_name, plot_color):
    player_event_x = [(ast.literal_eval(player_event)[0]['x'] * 104) / 100. for player_event in player_events]
    player_event_y = [(ast.literal_eval(player_event)[0]['y'] * 68) / 100. for player_event in player_events]

    p = draw_pitch(figure_width=450, figure_height=295, pitch_width=104, pitch_height=68, line_color="grey",
                   pitch_color="white")
    p.circle(player_event_x, player_event_y, fill_color=plot_color, line_width=1, line_color="grey", fill_alpha=0.2,
             size=8)
    player_stats = bokeh.models.Label(x=70, y=190, x_units='screen', y_units='screen',
                                      text=str(len(player_event_x)) + " " + event_name, text_font_size='20px',
                                      render_mode='css',
